...
describe("synchronized methods", () => {
  it("should not synchronize between instances", () => {
    ...
    return Promise.all([t0.fn(0), t1.fn(0), t0.fn(2)]).then(() => {
      expect(i).toBe(3);
    });
  });
...
describe("synchronized functions with keys", () => {
  it("should synchronize async functions", async () => {
    const counters = [];
    const fn = synchronized.withKey()(async (key, result) => {
      counters[key] = (counters[key] || 0) + 1;
      return result;
    });

    const p0 = Promise.all([fn(0, "foo"), fn(0, "bar")]);
    const p1 = Promise.all([fn(1, "baz"), fn(1, "qux")]);
